/**
 * Server-side post-quantum vault cryptography.
 * Uses ML-KEM768 for key encapsulation and AES-256-GCM for symmetric encryption.
 * The ML-KEM private key is encrypted at rest using a server secret.
 */
...
async function deriveServerKey(): Promise<CryptoKey> {
  const encoder = new TextEncoder();
  const keyMaterial = encoder.encode(env.SERVER_SECRET);
...
export async function hybridEncapsulate(
  ...
  return { ciphertext: cipherText, sharedSecret: hybridSecret };
}
...
export async function encryptWithServerSecret(plaintext: Uint8Array): Promise<{
  ciphertext: string;
  iv: string;
  authTag: string;
}> {
...
export async function decryptWithServerSecret(
  ciphertext: string,
  iv: string,
  authTag: string
): Promise<Uint8Array> {
...
export function generateClassicalSecret(): Uint8Array {
  return crypto.getRandomValues(new Uint8Array(32));
}
